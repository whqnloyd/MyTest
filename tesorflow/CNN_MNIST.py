...
#2层卷积层
w_conv1 = weight_variable([5, 5, 1, features1])                  #前两维为patch的大小，输入的通道数目，输出的通道数目（特征数）
b_conv1 = bias_variable([features1])                             #每一个输出通道都有一个对应的偏置量
...
#密集连接层
w_dc = weight_variable([7 * 7 * features2, dc_layer])               #尺寸减小到7x7，加入一个全连接层
b_dc = bias_variable([dc_layer])

pool_flat = tf.reshape(pool2, [-1, 7 * 7 * features2])
...
#成本评估
loss = cross_entropy = -tf.reduce_sum(labels*tf.log(y))
